...
from .src.modeling_bertX import BertLayerNorm, GeLU, BertLayer
from .src.modeling_bertU import BertU
from .src.modeling_robertaU import RobertaU

from .src.transformers.tokenization_auto import AutoTokenizer
...
class ModelU(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.max_seq_len = 100 + args.num_feats
        self.num_features = args.num_feats
        self.tr_name = args.tr
        ...
        if self.model.config.type_vocab_size == 1:
            ...
            # Initialize it
            self.model.embeddings.token_type_embeddings.weight.data.normal_(mean=0.0,
    ...
    def forward(self, sents, visual_feats):
        ...
        if self.tr_name.startswith("roberta"):
            input_ids, img_feats, img_pos_feats, attn_masks, gather_index = self.preprocess_roberta(sents, visual_feats,
                                                                                                    self.num_features,
                                                                                                    self.tokenizer)
        elif self.tr_name.startswith("bert"):
            input_ids, img_feats, img_pos_feats, attn_masks, gather_index = self.preprocess_bert(sents, visual_feats,
                                                                                                 self.num_features,
                                                                                                 self.tokenizer)

        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        seq_out, pooled_output = self.model(input_ids.to(device), None, img_feats.to(device), img_pos_feats.to(device),
                                            attn_masks.to(device), gather_index=gather_index.to(device))
    ...
    def load(self, path):
        ...
        for key, value in state_dict.items():
            ...
            if key.startswith("img_embeddings.pos_linear.weight"):
                if self.num_pos == 6:  # Loading all 7 (not 6) for UNITER
