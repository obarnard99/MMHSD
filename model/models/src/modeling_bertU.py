...
class UniterImageEmbeddings(nn.Module):
    def __init__(self, config, img_dim, num_pos):
        super().__init__()
...
class BertU(BertPreTrainedModel):
    ...
    def __init__(self, config, img_dim=2048, num_pos=6):
        super().__init__(config)
        self.embeddings = UniterTextEmbeddings(config)
        self.img_embeddings = UniterImageEmbeddings(config, img_dim, num_pos)
